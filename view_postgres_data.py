...
def view_data():
    ...
    try:
        ...
        print("="*60)
        cur.execute("SELECT race_id, event_name, start_date, operator FROM events ORDER BY create_at DESC LIMIT 5")
        events = cur.fetchall()
        if not events:
            print("Belum ada data event.")
        for ev in events:
            indicator = ">>> " if ev['race_id'] == active_id else "    "
            print(f"{indicator}Nama: {ev['event_name'] or 'none'} | ID: {ev['race_id']} | Op: {ev['operator']}")
        ...
        query = "SELECT no_start, line_status, time_stamp, ss, send, create_at FROM timing WHERE race_id = %s"
        params = [active_id]
        ...
        if not timings:
            ...
            print(f"{'NS':<5} | {'LINE':<6} | {'TIME STAMP':<15} | {'SS':<3} | {'SYNC':<5} | {'CREATED AT'}")
            print("-" * 75)
            for t in timings:
                ...
                print(f"{str(t['no_start']):<5} | {t['line_status']:<6} | {t['time_stamp']:<15} | {t['ss']:<3} | {sync_status:<5} | {created_at}")
